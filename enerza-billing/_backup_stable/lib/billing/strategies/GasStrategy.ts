...
export class GasStrategy extends BaseBillingStrategy {
  utility = "GAS_PNG";
  ...
  calculate(plan: RatePlanDef, params: RateParams): RatingResult {
    ...
    for (const c of plan.components) {
      ...
      if (c.type === "VARIABLE" || c.type === "APM_QUOTA") {
    ...
    // 2. LNG Blend Surcharge (Gas Phase 2 Logic)
    for (const c of plan.components) {
      if (c.type === "LNG_SURCHARGE") {
        const maxSlabTo = Math.max(...plan.components
          .filter(comp => comp.type === "VARIABLE" || comp.type === "APM_QUOTA")
          .map(comp => comp.slabTo ?? 0)
        );
        
        if (consumption > maxSlabTo) {
          const excess = consumption - maxSlabTo;
          const amount = this.r2(excess * c.rate);
          amounts[c.id] = amount;
          netAmount += amount;
          lines.push({ componentId: c.id, name: c.name, type: "CHARGE", qty: this.r2(excess), uom: plan.uom, rate: c.rate, amount });
        }
      }
    }
