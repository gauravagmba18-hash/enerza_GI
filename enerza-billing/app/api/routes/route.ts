...
export async function GET(req: NextRequest) {
  try {
    ...
    const where: any = search ? { OR: [{ routeName: { contains: search } }, { cycleGroup: { contains: search } }] } : {};
    const [data, total] = await Promise.all([
      (prisma.route as any).findMany({ where, skip, take: limit, orderBy: { createdAt: "desc" } }),
